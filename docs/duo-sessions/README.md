...
知見は 3 つの層に分けて扱います。所有者と更新者を分けることが目的です。

| 層 | 置き場所 | 所有 | 更新者 |
|----|----------|------|--------|
| **生ログ層** | `docs/duo-sessions/YYYY-MM-DD.md` | 記録 | エージェント（append-only） |
| **蒸留層** | `LEARNED.md` | エージェント | エージェント |
| **恒久仕様層** | `CLAUDE.md`、`CONTRIBUTE.md`、`ARCHITECTURE.md` | 人間 | 人間のみ |

生ログを貯めるだけでは読み返されません。**生産性が上がるのは蒸留層への還流**です。
振り返りのたびに「これは次回も起きるか？」を問い、Yes なら `LEARNED.md` に書きます。

振り返りから得た学びを `CLAUDE.md` へ直接書き込まないでください。
`CLAUDE.md` はプロダクト仕様の単一情報源で、人間が所有します。
`LEARNED.md` の項目が十分に水平的・永続的であると人間が判断したときにだけ、
人間が恒久仕様層へ昇格させます。

蒸留層の更新ルール（根拠必須、昇格閾値、上限件数、失効）は `LEARNED.md` の冒頭にあります。
...
4. 繰り返し現れた摩擦を `LEARNED.md` の Keep / Problem / Try / Notes に反映する
