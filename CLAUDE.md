...
エージェントがセッション振り返りから学んだ運用上の知見は、このファイルでなく
`LEARNED.md`に集約します。Keep / Problem / Try / Notesの4区分で、振り返りのたびに
エージェントが更新します。根拠となるセッションの生ログは`docs/duo-sessions/`にあります。

`CLAUDE.md`はプロダクト仕様の単一情報源であり、人間が所有します。
振り返りで得た知見を直接ここへ書き込まないでください。
