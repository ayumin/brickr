...
この文書は、`main` ブランチを起点にフェーズ1の実装を進めるための実装仕様書である。各Stepの作業ブランチは `main` から作成し、独立MRとして `main` へマージする。
...
4. rootが自分の投稿を引用（`quoteOf`）している

「自分への返信」は、親Postのauthorが自分であるreplyを指す。単に同じthreadへ参加しているだけでは含めない。引用リポストは常に独立rootであり（§12.1, §8.3）、条件4はroot自身についての事実である。

`自分あて`表示時の返信プレビュー（§12.2の最新2件）は、自分に関係する返信（条件2・3を満たすreply）が新しい無関係な返信に押し出されて消えないよう、最新2件から漏れていれば1件バックフィルする。バックフィルする場合、既存プレビューのうち最新の1件と入れ替え、古い→新しい順を保つ。
