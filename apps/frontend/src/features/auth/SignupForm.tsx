...
import { MIN_PASSWORD_LENGTH, type AuthUserDto, type SignupRequest } from "@brickr/shared";
...
export function SignupForm({ onSuccess }: SignupFormProps) {
  ...
  const submit = async (): Promise<void> => {
    ...
    setError(null);
    try {
      const user = await api.signup(toSignupRequest(form));
      onSuccess(user);
    } catch (cause) {
  ...
  return (
    ...
    >
      {error ? <ErrorBanner message="登録できませんでした" detail={error} onDismiss={() => setError(null)} /> : null}

      <TextField label="招待コード" value={form.inviteCode} required onChange={setField("inviteCode")} />
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        />
        <TextField label="表示名" value={form.displayName} required onChange={setField("displayName")} />
        <TextField label="生年月日" type="date" value={form.birthdate} required onChange={setField("birthdate")} />
      </div>
      ...
      <div className="space-y-4 border-t border-line pt-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <TextField label="地域（都道府県 / 州）" value={form.region} onChange={setField("region")} />
          <TextField label="職業" value={form.occupation} onChange={setField("occupation")} />
          <TextField label="X (Twitter) ハンドル" prefix="@" value={form.xHandle} onChange={setField("xHandle")} />
          ...
          <div className="sm:col-span-2">
            <TextField label="自己紹介" value={form.description} onChange={setField("description")} />
          </div>
        </div>
      </div>
